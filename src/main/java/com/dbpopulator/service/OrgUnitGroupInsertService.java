...
@Service
public class OrgUnitGroupInsertService {

    private static final Logger log = LoggerFactory.getLogger(OrgUnitGroupInsertService.class);
    ...
    public OrgUnitGroupInsertService(DataSource dataSource,
    ...
    public int insertOrgUnitGroups(int amount, ProgressCallback callback) throws SQLException {
        log.info("Starting orgunitgroup insert: {} rows", amount);

        TableMetadata table = schemaService.getTable("orgunitgroup");
        if (table == null) {
            throw new IllegalArgumentException("Table not found: orgunitgroup");
        }
        List<ColumnMetadata> columns = table.getInsertableColumns();
        String sql = buildInsertSql("orgunitgroup", columns);
        ...
        try (Connection conn = dataSource.getConnection()) {
            ...
            try {
                ...
                log.info("Starting orgunitgroupid from {}", nextId);

                try (PreparedStatement ps = conn.prepareStatement(sql)) {
                    for (int i = 0; i < amount; i++) {
                        Map<String, Object> row = dataGenerator.generateRow(table);
                        row.put("orgunitgroupid", nextId + i);
                        row.put("name", "OrgUnitGroup-" + dataGenerator.generateRandomSuffix());
                ...
                conn.rollback();
                log.error("OrgUnitGroup insert failed, rolling back", e);
                throw e;
            }
        }

        log.info("OrgUnitGroup insert complete: {} rows inserted", inserted);
        return inserted;
    }

    private long getMaxPkValue(Connection conn) throws SQLException {
        String sql = "SELECT COALESCE(MAX(orgunitgroupid), 0) FROM orgunitgroup";
        try (PreparedStatement ps = conn.prepareStatement(sql);
